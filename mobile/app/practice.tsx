  ...
  return <>
    ...
    <View style={s.builder}>
      ...
      {mode === "words" ? <Text style={s.count}>{wordIds.length} selected</Text> : <>
        <View style={s.row}>{[5, 10, 20].map((n) => <Chip key={n} active={cardCount === n} label={`${n}`} press={() => setCardCountInput(String(n))} />)}</View>
        <View style={s.customCountRow}><Text style={s.optionTitle}>Custom amount</Text><TextInput accessibilityLabel="Custom card amount" keyboardType="number-pad" maxLength={3} value={cardCountInput} onChangeText={(value) => setCardCountInput(value.replace(/\D/g, ""))} onBlur={() => setCardCountInput(String(Math.min(100, Math.max(1, Number(cardCountInput) || 1))))} style={s.countInput} /><Text style={s.count}>1–100</Text></View>
        {!validCardCount && <Text style={s.error}>Enter a number from 1 to 100.</Text>}
      </>}
...
const s = StyleSheet.create({
  ...
  search: { backgroundColor: "#f7f3eb", borderColor: "#ded5c4", borderRadius: 14, borderWidth: 1, color: "#173b38", fontSize: 15, marginBottom: 10, padding: 14 }, count: { color: "#63716d", fontSize: 12, marginVertical: 5 }, label: { color: "#9a431f", fontSize: 11, fontWeight: "800", letterSpacing: 1.5, marginTop: 24, marginBottom: 9 }, row: { alignItems: "center", flexDirection: "row", flexWrap: "wrap", gap: 8 }, customCountRow: { alignItems: "center", flexDirection: "row", gap: 10, marginTop: 12 }, countInput: { backgroundColor: "#f7f3eb", borderColor: "#cfc6b7", borderRadius: 10, borderWidth: 1, color: "#173b38", fontSize: 16, fontWeight: "700", paddingHorizontal: 12, paddingVertical: 8, textAlign: "center", width: 68 }, chip: { borderColor: "#cfc6b7", borderRadius: 99, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 9 }, chipActive: { backgroundColor: "#173b38", borderColor: "#173b38" }, chipText: { color: "#52645f", fontSize: 13, fontWeight: "700" }, chipTextActive: { color: "#fffdf8" }, disabled: { opacity: 0.35 },
