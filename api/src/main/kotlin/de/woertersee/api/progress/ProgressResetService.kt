...
@Service class ProgressResetService(private val jdbc:JdbcClient){
 @Transactional fun reset(profileId:UUID,request:ResetProgressRequest):ResetProgressResult{
  ...
  val affected=when(request.type){
   ResetType.UNSEEN_HISTORY->jdbc.sql("UPDATE user_progress SET state='UNSEEN',consecutive_known=0,review_interval_minutes=0,next_review_at=now(),updated_at=now(),version=version+1 WHERE profile_id=:id").param("id",profileId).update()+jdbc.sql("UPDATE personal_word_progress SET state='UNSEEN',consecutive_known=0,review_interval_minutes=0,next_review_at=now(),updated_at=now(),version=version+1 WHERE profile_id=:id").param("id",profileId).update()
